...
import com.example.ludioil.ludihexiao.adapter.CouponAdapter;
...
public class CouponActivity extends AppCompatActivity {
    ...
    private void getData() {
        datas.clear();
        OkGo.<String>post(API_URL+"?token="+login_token+"&request=private.coupon.get.success.fluid.list&platform=merchant_user_app&station_id="+station_id)
    ...
    private void showDetail(final int position){
        ...
        btn_fix.setText("核销补打小票");
        btn_fix.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                PrintTicket(datas.get(position).getId(),dialog);
            }
        });
        dialog.show();
    }
     ...
     * 补打小票接口
     */
    private void PrintTicket(String id, final Dialog dialog){
